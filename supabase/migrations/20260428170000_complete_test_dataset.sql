...
BEGIN
  SELECT id INTO v_baghdad_org
  FROM public.organizations
  WHERE name = 'Baghdad Test Legal'
  ORDER BY created_at
  LIMIT 1;

  SELECT id INTO v_erbil_org
  FROM public.organizations
  WHERE name = 'Erbil Test Legal'
  ORDER BY created_at
  LIMIT 1;

  SELECT id INTO v_admin
  FROM public.profiles
  WHERE email = 'yousif.alnajjar@baghdadtest.qanuni'
  LIMIT 1;

  SELECT id INTO v_erbil_admin
  FROM public.profiles
  WHERE email = 'admin@erbiltest.qanuni'
  LIMIT 1;
  ...
  FROM (
    SELECT id, row_number() OVER (ORDER BY company_registration_number) rn
    FROM public.entities
  ...
  -- Link all portal users to Baghdad people. Link clients 1-3 to Erbil too
  -- so the portal org picker and org switcher have real data.
  ...
  IF v_erbil_org IS NOT NULL THEN
    ...
    JOIN LATERAL (
      SELECT id
      FROM public.persons
      WHERE organization_id = v_erbil_org
        AND tags @> ARRAY['seed:phase2']
      ORDER BY (regexp_replace(email, '[^0-9]', '', 'g'))::int
      OFFSET ((regexp_replace(pu.email, '[^0-9]', '', 'g'))::int - 1)
      LIMIT 1
  ...
  SELECT v_baghdad_org, c.id, CURRENT_DATE + (rn % 30),
         ('09:00'::time + ((rn % 5) || ' hours')::interval),
         (ARRAY['first_hearing','evidence','pleading','judgment'])[1 + (rn % 4)],
         'scheduled',
         'Room ' || (1 + (rn % 12)),
         'Auto-seeded hearing'
  FROM (
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM public.case_hearings h WHERE h.case_id = c.id
  );

  SELECT count(*) INTO v_errand_count
  FROM public.errands
  WHERE organization_id = v_baghdad_org
    AND description = 'Auto-seeded Phase 2 errand';
  ...
  FROM public.tasks
  WHERE organization_id = v_baghdad_org
    AND description = 'Auto-seeded Phase 2 task';
  ...
  WHERE organization_id = v_baghdad_org
    AND notes = 'Auto-seeded Phase 2 invoice';
