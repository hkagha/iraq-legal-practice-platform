-- Trust errand link + errand billing cleanup (commit 3362609)
...
BEGIN
  ...
  IF v_party_type = 'person' AND NOT EXISTS (
    ...
    WHERE p.id = v_person_id AND p.organization_id = v_org_id
  ) THEN
  ...
  IF v_party_type = 'entity' THEN
    IF NOT EXISTS (
      ...
      WHERE e.id = v_entity_id AND e.organization_id = v_org_id
    ) THEN
