...
const CATEGORIES = [
  { group: 'Tasks', groupAr: 'المهام', types: ['task_assigned', 'task_due_today', 'task_due_tomorrow', 'task_overdue', 'task_completed', 'task_comment', 'task_mentioned'] },
  { group: 'Cases', groupAr: 'القضايا', types: ['case_assigned', 'case_status_changed', 'case_hearing_tomorrow', 'case_hearing_today', 'case_note_added', 'case_mentioned', 'case_deadline_approaching'] },
  { group: 'Errands', groupAr: 'المعاملات', types: ['errand_assigned', 'errand_status_changed', 'errand_step_completed', 'errand_overdue', 'errand_mentioned'] },
  { group: 'Billing', groupAr: 'الفوترة', types: ['invoice_created', 'invoice_overdue', 'payment_received'] },
  { group: 'Documents', groupAr: 'المستندات', types: ['document_shared', 'document_uploaded'] },
  { group: 'Calendar', groupAr: 'التقويم', types: ['event_reminder', 'event_invitation'] },
  { group: 'Mentions', groupAr: 'الإشارات', types: ['mention'] },
];
...
export default function NotificationPrefsSection() {
  ...
  const save = useCallback((newPrefs: Record<string, any>, extras?: Record<string, any>) => {
    ...
    saveTimerRef.current = setTimeout(async () => {
      ...
      await supabase.from('notification_preferences').update({
        ...
        email_enabled: extras?.email_enabled ?? emailEnabled,
        whatsapp_enabled: extras?.whatsapp_enabled ?? whatsappEnabled,
        quiet_hours_enabled: extras?.quiet_hours_enabled ?? quietHoursEnabled,
        quiet_hours_start: extras?.quiet_hours_start ?? quietStart,
        quiet_hours_end: extras?.quiet_hours_end ?? quietEnd,
        sound_enabled: extras?.sound_enabled ?? soundEnabled,
        ...extras,
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-heading-lg text-foreground">{t('settings.sections.notificationPrefs')}</h2>
          ...
          <div key={cat.group}>
            ...
            {cat.types.map(type => (
              <div key={type} className="grid grid-cols-[1fr_60px_60px_60px] gap-2 py-1.5 items-center">
                <span className="text-body-sm text-foreground">{language === 'ar' ? TYPE_LABELS[type]?.ar : TYPE_LABELS[type]?.en}</span>
                <div className="flex justify-center"><Checkbox checked={getPref(type, 'in_app')} onCheckedChange={() => togglePref(type, 'in_app')} /></div>
                <div className="flex justify-center"><Checkbox checked={getPref(type, 'email')} onCheckedChange={() => togglePref(type, 'email')} disabled={!emailEnabled} /></div>
                <div className="flex justify-center"><Checkbox checked={getPref(type, 'whatsapp')} onCheckedChange={() => togglePref(type, 'whatsapp')} disabled={!whatsappEnabled} /></div>
              </div>
            ))}
          </div>
        ))}
