...
export default function DocumentsPage() {
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    let query = supabase
      ...
      .select(`
        *,
        uploader:profiles!documents_uploaded_by_fkey(id, first_name, last_name, first_name_ar, last_name_ar, avatar_url),
        client:clients(id, first_name, last_name, first_name_ar, last_name_ar, company_name, company_name_ar, client_type),
        case:cases(id, case_number, title, title_ar),
        errand:errands(id, errand_number, title, title_ar)
      `, { count: 'exact' })
    ...
    // Search
    if (searchQuery) {
      query = query.or(`file_name.ilike.%${searchQuery}%,file_name_ar.ilike.%${searchQuery}%,title.ilike.%${searchQuery}%,title_ar.ilike.%${searchQuery}%,description.ilike.%${searchQuery}%`);
    }

    // Category filter
    if (activeFilters.category && activeFilters.category !== 'all') {
      query = query.eq('document_category', activeFilters.category);
    }
    ...
    if (activeFilters.fileType && activeFilters.fileType !== 'all') {
      switch (activeFilters.fileType) {
        ...
        case 'other': query = query.not('file_type', 'in', '("pdf","doc","docx","xls","xlsx","jpg","jpeg","png","gif","webp")'); break;
      }
    }
  ...
  const toggleVisibility = async (doc: any, e: React.MouseEvent) => {
    ...
    const { error } = await supabase.from('documents').update({ is_visible_to_client: newVal }).eq('id', doc.id);
  ...
  const getLinkedEntity = (doc: any) => {
    if (doc.case) {
      return { icon: Scale, label: doc.case.case_number, href: `/cases/${doc.case.id}` };
    }
    if (doc.errand) {
      return { icon: FileCheck, label: doc.errand.errand_number, href: `/errands/${doc.errand.id}` };
    }
    if (doc.client) {
      const name = language === 'ar'
        ? (doc.client.client_type === 'company' ? doc.client.company_name_ar || doc.client.company_name : `${doc.client.first_name_ar || doc.client.first_name} ${doc.client.last_name_ar || doc.client.last_name}`)
        : (doc.client.client_type === 'company' ? doc.client.company_name : `${doc.client.first_name} ${doc.client.last_name}`);
  ...
  const getUploaderName = (doc: any) => {
    ...
    return language === 'ar'
      ? `${doc.uploader.first_name_ar || doc.uploader.first_name} ${doc.uploader.last_name_ar || doc.uploader.last_name}`
  ...
  const columns = [
      ...
      render: (row: any) => {
        ...
        return (
          <div className="flex items-center gap-2.5 min-w-0">
            <FIcon size={20} style={{ color }} className="shrink-0" />
            <div className="min-w-0">
              <p className="text-body-md font-medium truncate">{language === 'ar' ? (row.title_ar || displayName) : displayName}</p>
              {showSubName && <p className="text-body-sm text-muted-foreground truncate">{showSubName}</p>}
            </div>
          </div>
        );
      },
    },
    {
      ...
      render: (row: any) => (
        <span className="inline-flex items-center text-xs font-medium rounded-badge px-2.5 py-0.5 bg-muted text-muted-foreground capitalize">
          {t(`documents.categories.${row.document_category}`)}
        </span>
      ),
    },
    {
      ...
      render: (row: any) => {
        ...
        return (
          <button onClick={(e) => { e.stopPropagation(); navigate(linked.href); }} className="flex items-center gap-1.5 text-accent hover:underline text-body-sm">
            <LIcon size={14} />
    ...
    },
    {
      ...
      render: (row: any) => (
        <span className={`text-xs font-medium rounded-badge px-1.5 py-0.5 ${row.version > 1 ? 'bg-accent/10 text-accent' : 'bg-muted text-muted-foreground'}`}>
          v{row.version}
        </span>
      ),
    },
    {
      ...
      render: (row: any) => (
        <button onClick={(e) => toggleVisibility(row, e)} className="p-1 rounded hover:bg-muted transition-colors">
          {row.is_visible_to_client
      ...
      render: (row: any) => (
        <DropdownMenu>
          <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
            <button className="p-1 rounded hover:bg-muted transition-colors"><MoreVertical size={16} /></button>
          </DropdownMenuTrigger>
  ...
  const filters = [
    {
      ...
      options: CATEGORY_OPTIONS.map(c => ({ value: c, label: t(`documents.categories.${c}`), labelAr: t(`documents.categories.${c}`) })),
    },
    {
      key: 'linkedTo', label: 'Linked To', labelAr: 'مرتبط بـ',
      options: [
        { value: 'cases', label: 'Cases', labelAr: 'القضايا' },
        { value: 'errands', label: 'Errands', labelAr: 'المعاملات' },
        { value: 'clients', label: 'Clients', labelAr: 'العملاء' },
        { value: 'unlinked', label: 'Unlinked', labelAr: 'غير مرتبط' },
      ],
    },
    {
      key: 'fileType', label: 'File Type', labelAr: 'نوع الملف',
      options: [
        { value: 'pdf', label: 'PDF', labelAr: 'PDF' },
        { value: 'word', label: 'Word', labelAr: 'Word' },
        { value: 'excel', label: 'Excel', labelAr: 'Excel' },
        { value: 'images', label: 'Images', labelAr: 'صور' },
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {documents.map((doc) => {
          ...
          return (
            <div
              key={doc.id}
              className="bg-card rounded-lg border border-border overflow-hidden cursor-pointer hover:shadow-md hover:border-muted-foreground/30 transition-all"
              onClick={() => {/* future: detail page */}}
            >
              <div className="h-[120px] bg-muted/50 flex items-center justify-center">
                {isImage ? (
                  <Image size={48} style={{ color }} />
                ) : (
                  <FIcon size={48} style={{ color }} />
                )}
              </div>
              <div className="p-3">
                <p className="text-body-md font-medium truncate">{displayName}</p>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[11px] font-medium rounded-badge px-1.5 py-0.5 bg-muted text-muted-foreground">
                    {t(`documents.categories.${doc.document_category}`)}
                  </span>
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <button onClick={(e) => toggleVisibility(doc, e)} className="p-0.5">
                    {doc.is_visible_to_client ? <Eye size={14} className="text-accent" /> : <EyeOff size={14} className="text-muted-foreground/40" />}
                  </button>
                </div>
              </div>
            </div>
  ...
  return (
    <div>
      <PageHeader
        ...
        titleAr={t('documents.title')}
        subtitle={t('documents.subtitle')}
        subtitleAr={t('documents.subtitle')}
        actionLabel={t('documents.upload')}
        actionLabelAr={t('documents.upload')}
        onAction={() => {/* upload modal - future */}}
        secondaryActions={[
          {
            label: t('documents.newFromTemplate'),
            labelAr: t('documents.newFromTemplate'),
            icon: FileText,
            onClick: () => {/* template modal - future */},
          },
        ]}
        breadcrumbs={[
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <StatCard
          ...
          iconBgColor="hsl(214, 100%, 97%)"
          label={t('documents.totalDocuments')}
          labelAr={t('documents.totalDocuments')}
          value={totalCount}
        />
        <StatCard
          icon={Upload}
          iconColor="hsl(142, 71%, 45%)"
          iconBgColor="hsl(138, 76%, 97%)"
          label={t('documents.recentUploads')}
          labelAr={t('documents.recentUploads')}
          value={recentCount}
        />
        <StatCard
          icon={HardDrive}
          iconColor="hsl(42, 50%, 54%)"
          iconBgColor="hsl(42, 52%, 95%)"
          label={t('documents.storageUsed')}
          labelAr={t('documents.storageUsed')}
      ...
      {/* Filters */}
      <FilterBar
        searchPlaceholder={t('documents.search.placeholder')}
        searchPlaceholderAr={t('documents.search.placeholder')}
